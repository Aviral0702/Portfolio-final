  ...
  <>
    <div className="relative min-h-screen w-full overflow-hidden pt-20">
      <div className="absolute inset-0 w-full h-full">
        <div className="absolute inset-0 bg-gradient-to-br from-spotify-dark via-spotify-dark-secondary to-spotify-dark-tertiary opacity-90" />
        <img
          src={bg}
          alt=""
      ...
      <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-spotify-green/5 rounded-full blur-3xl motion-safe:animate-pulse-slow" style={{ animationDelay: "1s" }} />
      </div>
      ...
      <div className="relative z-10 min-h-screen px-2.5 flex items-center justify-center">
        <div className="flex flex-col md:flex-row items-center justify-between max-w-6xl w-full gap-8">
          ...
          >
            ...
            <p className="text-xl sm:text-2xl md:text-3xl mb-6 text-spotify-text-secondary font-medium">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 mb-6">
              {metrics.map((metric) => (
                <div
                  key={metric.label}
                  className="card-spotify !p-3 text-center min-w-[120px]"
                >
                  <p className="text-lg font-bold text-spotify-green">{metric.value}</p>
                  <p className="text-xs text-spotify-text-secondary">{metric.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              ...
              >
                <img src={fileIcon} alt="" className="w-5 h-5 transition-transform group-hover:scale-110" aria-hidden="true" />
                Download Resume
              </a>
              <a href="#projects" className="btn-spotify-secondary flex items-center justify-center gap-3">
                View Projects
              </a>
            </div>
          ...
          >
            <div className="relative">
              <img
                ...
                className="w-[250px] sm:w-[300px] md:w-[400px] lg:w-[450px] h-auto object-contain motion-safe:animate-float"
              />
              <div className="absolute inset-0 w-full h-full bg-spotify-green/10 rounded-full blur-3xl motion-safe:animate-glow" />
            </div>
          </motion.div>
        </div>
      </div>
      ...
      >
        <div className="flex flex-col items-center text-spotify-text-tertiary">
          ...
          <div className="w-6 h-10 border-2 border-spotify-text-tertiary rounded-full flex justify-center">
            <div className="w-1 h-3 bg-spotify-text-tertiary rounded-full mt-2 motion-safe:animate-bounce" />
          </div>
        </div>
      </motion.div>
    </div>
